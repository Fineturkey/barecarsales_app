...
$purchase_id = (string) $repair['purchase_id'];
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        ...
        if (!$chk_res || $chk_res->num_rows === 0) {
            $errors[] = "That purchase does not exist. Choose a purchase from the list.";
        }
        $chk->close();
    }
...
<?php if (empty($purchases)): ?>
    <div class="message error">There are no purchases. <a href="/barecarsales_app/purchase/purchase_create.php">Add a purchase</a> first.</div>
<?php else: ?>
<form method="post">
