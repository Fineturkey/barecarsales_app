...
$stmt = $conn->prepare("
    SELECT payment_id, customer_id, sale_id, amount, payment_date, due_date, paid_date, bank_account, is_late, days_late
    FROM payment
    WHERE payment_id = ?
");
...
<form method="post" class="form-payment">
    ...
    <label>
        ...
        <input type="hidden" id="is_late" name="is_late" value="<?= $is_late ?>">
    </label>

    <label>
        Days Late: <span id="days_late_display"><?= htmlspecialchars($days_late ?? '0') ?></span>
        <input type="hidden" id="days_late" name="days_late" value="<?= htmlspecialchars($days_late ?? '0') ?>">
    </label>
